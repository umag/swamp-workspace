...
A colleague was driving the `webhook-retry-backoff` issue through its lifecycle
...
`inputs/session-state.json`.

The repository root is at `/workspace/webhooks-service`. You are currently
working from a Claude Code worktree at
`/workspace/webhooks-service/.claude/worktrees/webhook-retry-backoff`. The team
...
it through the next complete phase. The script should:
