...
Optional companion to [plan-presentation.md](plan-presentation.md). Read it when
a plan is too large or too comparative for the terminal render.
...
The escalation should fire **on its own**, not require the human to request it.
Heuristic the issue-lifecycle harness uses:
...
- **Strategic context section** (optional) — a Wardley map when alternatives sit
  in different evolution quadrants ("write our own cache" vs "managed Redis").
  See [plan-wardley.md](plan-wardley.md). Wardley lives **only** in HTML — the
  CLI doesn't render Mermaid.
...
- Risks) so the decision can start in the terminal and only drop to the browser
  when the reader wants depth.
