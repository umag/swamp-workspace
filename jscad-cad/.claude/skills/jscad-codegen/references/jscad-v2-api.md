...
All examples tested against `@jscad/modeling@2.12.0`. Source: official JSCAD
tutorials and source code.

> **In swamp `@jscad/cad` scripts**: no `require()` or `import` needed. The
> following names are injected into scope automatically: `primitives`,
> `transforms`, `booleans`, `expansions`, `extrusions`, `hulls`, `measurements`,
> `text`, `colors`
...
All transforms are **pure functions** — they return a new geometry and never
modify the input.
...
Always make the cutter **slightly taller** than the base to avoid co-planar
faces:
