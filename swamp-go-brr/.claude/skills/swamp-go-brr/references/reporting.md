...
`swamp model method run <run> hydrate` → the `summary` resource: `status`,
per-status `buckets`, `invocations/maxInvocations`, `leased` (id+vmId),
`waitingFollowups`, `stallCulprits`, `stallSignature`, `costEstimate`
(advisory), `haltReason`, `haltOptions`.
...
Every halt yields a non-empty `haltReason` + numbered `haltOptions`. Present
them verbatim and hand to the human (sacred rule 4). The cause set is closed:
...
- **infra_error** — firecracker/docker-verify host trouble (or version
  mismatch):
...
claude_error / out_of_allowlist / transport / oversize) tells "the agent
produced garbage" apart from a plain test failure.
