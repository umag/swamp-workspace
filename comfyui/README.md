...
**structured JSON caption** with per-object **bounding boxes** — so you can place
named objects and text in precise regions of the frame.
...
`anthropicApiKey` accepts a standard Anthropic API key **or** a Claude Code OAuth
token (`sk-ant-oat…`); the client auto-selects `x-api-key` vs OAuth Bearer auth.
...
*Export (API)* menu. In the Ideogram-4 template the caption node is
