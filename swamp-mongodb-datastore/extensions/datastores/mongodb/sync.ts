...
export function createSyncService(
  ...
  async function pull(opts: {
    ...
    // A non-scoped pull probes the global watermark first — a cheap findOne
    // that returns nothing when the whole tier is quiet. A scoped pull skips
    // the probe: its per-prefix floors already narrow the main cursor to new
    // docs, and that cursor is the single query the pull costs.
    ...
    const baseFilter: Record<string, unknown> = scoped
      ? scopedPullFilter(prefixList, state)
    ...
    // Per-prefix max updatedAt seen — the scope watermarks this pull records.
    ...
    for await (const doc of paths.find(filter)) {
      ...
      if (scoped) {
        const owner = prefixList.find((p) => rel.startsWith(p));
        if (owner !== undefined) {
          const cur = scopeMaxMs.get(owner);
          if (cur === undefined || ms > cur) scopeMaxMs.set(owner, ms);
        }
      }
    ...
    if (scoped && !metadataOnly) {
      // Each scoped prefix is now hydrated up to the max updatedAt seen for it,
      // or its prior floor when nothing matched. Recorded per prefix, never as
      // lastPulledAt: this pull saw one slice of the manifest, not the whole.
...
// `prefix` — the floor a scoped pull filters on. The later of two facts:
// `scopeWatermarks[prefix]` is what a previous scoped pull of this prefix
// established, and `lastPulledAt` also qualifies because only a full,
// non-metadataOnly, unscoped pull sets it and that pull hydrated everything.
// Neither alone is enough: `lastPulledAt` is untouched by scoped pulls (which
// core issues for nearly every pull once scopedSync is advertised), while a
// brand-new prefix has no scope watermark but a full pull may already cover it.
...
// locks a hot model and a dormant one doesn't rescan the dormant one. A prefix
// with no floor matches everything beneath it — the pre-watermark behavior,
// paid once, after which the pull records a floor.
