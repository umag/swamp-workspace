...
// place of a count for any method listed here.
...
//     hand-rolled ctx (e.g. makeSyncCtx below) has to opt in.
...
Deno.test("SSRF FIX: fetchPage(bandcampUrl) rejects a non-Bandcamp host (internal/metadata-service address) BEFORE any fetch — zero network egress", async () => {
  ...
  await withFetchStub(
    ...
    async (calls) => {
      ...
      // Zero egress alone doesn't rule out a write made before the
      // rejection (round 5 review, HIGH 2) — assert zero PERSISTENCE too,
      // matching the sibling SSRF test below and the funnel's own
      // throwing-exit check on "seed-from-bandcamp" (expected 0 writes),
      // but pinned locally so this security guarantee reads standalone.
      assertEquals(
        written.length,
        0,
        "the internal-host rejection must also write nothing — zero persistence, not just zero egress",
      );
    },
  );
});
    ...
    (calls) =>
      drainAndAwait(
        ...
        assertRejects(
          ...
          // Zero egress alone doesn't rule out a write made before the
          // rejection (round 5 review, HIGH 2) — assert zero PERSISTENCE
          // too, matching the funnel's own throwing-exit check on
          // "find-missing" (expected 0 writes) but pinned locally so this
          // security guarantee reads standalone.
          assertEquals(
            written.length,
            0,
            "the internal-host rejection must also write nothing — zero persistence, not just zero egress",
...
function makeSyncCtx(
  ...
  return {
    ...
    ctx: {
      ...
      logger: { info: () => {}, warning: () => {} },
      // See makeCtx() above — kept here too even though this file's only
      // caller of makeSyncCtx() is the shape-checked (not count-budgeted)
      // sync-artist-discographies, so opting in costs nothing and any future
      // budgeted use is covered.
      __written: written,
    },
  };
}
