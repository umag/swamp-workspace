...
[1-2 paragraph summary extracted from ARC-{PROJECT_ID}-REQ-v*.md Business
Requirements section - describe what needs to be delivered and why]

### 1.2 Strategic Alignment

**Architecture Principles**: [Reference relevant principles from
ARC-000-PRIN-v*.md that constrain this procurement]

**Stakeholder Priorities** (if ARC-{PROJECT_ID}-STKE-v*.md exists): [List top 3
stakeholder drivers/goals this addresses with IDs: D-001, G-001, etc.]

### 1.3 Expected Outcomes

[Extract from ARC-{PROJECT_ID}-REQ-v*.md Business Requirements (BR-xxx) - the
measurable outcomes]
...
[Describe what vendor must deliver - the complete deliverable or specific
outcome]

**What Success Looks Like**:

[Extract success criteria from ARC-{PROJECT_ID}-REQ-v*.md - ensure
technology-agnostic]
...
[Extract from ARC-{PROJECT_ID}-REQ-v*.md - what capabilities are absolutely
required]
...
- **Security**: [Skills for NFR-S-xxx requirements, reference security
  principles]
- **Performance**: [Skills for NFR-P-xxx requirements]
- **Compliance**: [Skills for NFR-C-xxx requirements, reference compliance
  principles]
...
- **[Principle Category]**: Experience with [specific technology/approach
  mandated by principles]
- **Design Reviews**: Experience with HLD/DLD review processes
- **Documentation**: Ability to produce architecture diagrams (Mermaid, C4)
- **Traceability**: Experience maintaining requirements traceability throughout
  delivery
...
[Extract user personas and scenarios from ARC-{PROJECT_ID}-REQ-v*.md to help
vendors understand context]

**User Personas**: [List personas from Functional Requirements section]
...
[Extract from ARC-{PROJECT_ID}-REQ-v*.md scope section OR infer from MUST
requirements]
...
[Extract from ARC-{PROJECT_ID}-REQ-v*.md OR infer from explicitly excluded
items]
...
Reference: Run `/arckit:hld-review` and `/arckit:dld-review` for formal review
processes
...
Reference: `/arckit:traceability` for traceability matrix generation and
validation
...
- **Technical Approach** (20%): Quality of proposed solution, alignment with
...
- **Track Record** (10%): Relevant project experience, client references,
  success stories
- **Team Structure** (5%): Appropriate roles, seniority levels, availability
  commitment

### 11.3 Quality Assurance (20%)

- **Testing Approach** (10%): Test coverage strategy, automation, non-functional
  testing
- **Compliance & Security** (5%): Security testing approach, compliance
  validation methods
...
- **Cost Breakdown** (5%): Transparency, justification, flexibility, no hidden
  costs
- **Risk Mitigation** (5%): Approach to project risks, contingency planning,
  issue management
...
- [ ] **Deployment runbooks**
- [ ] **Training materials**
- [ ] **Requirements traceability matrix** (Requirements → Design → Tests →
      Code)
...
4. **Answer Supplier Questions**: Via Digital Marketplace platform (visible to
   all)
5. **Evaluate Proposals**: Using criteria in Section 11
6. **Conduct Assessments**: Interview/technical assessment for shortlisted
   suppliers
...
- **Requirements**: projects/[project]/ARC-_-REQ-v_.md
- **Architecture Principles**: projects/000-global/ARC-000-PRIN-v*.md
- **Stakeholder Analysis**: projects/[project]/ARC-_-STKE-v_.md (if exists)
- **General RFP/SOW**: projects/[project]/procurement/ARC-_-SOW-_.md (if exists)

### 15.3 ArcKit Commands for Vendor Management

- **`/arckit:evaluate`**: Create vendor evaluation framework and scoring
- **`/arckit:hld-review`**: High-Level Design review process for vendor
  deliverables
- **`/arckit:dld-review`**: Detailed Design review process for vendor
  deliverables
...
- No changes to requirements after publishing (unless necessary and communicated
  to all)
...
**Generated by**: ArcKit `/arckit:dos` command **Generated on**: [DATE] **ArcKit
