...
**Purpose**: {1-2 paragraphs describing what this data product provides, who
uses it, and why it exists}

**Business Value**: {How this data product drives business outcomes or supports
critical processes}
...
**Objects**: {N} objects defined **Properties**: {M} total properties across all
objects

**ODCS Terminology**:

- **Object** = Structure of data (table in RDBMS, document in NoSQL, file in
  data lake)
...
- **Dual Running Period**: {N days} where both old and new versions are
  supported
...
**Backward Compatibility Commitment**: {All MINOR and PATCH releases are
backward compatible}
...
```yaml
rule_type: {
  row_count / null_check / uniqueness / referential_integrity / regex / range / custom,
}
...
**Quality Incident Response**: {Process for handling quality incidents, who to
...
**Freshness Monitoring**: {How freshness is measured - last update timestamp per
partition/record}
...
4. **Provisioning**: Credentials and endpoint details sent within {1 business
   day}
...
**Authorization Model**: {Role-Based Access Control (RBAC) / Attribute-Based
Access Control (ABAC)}
...
**Log Retention**: {1 year} in hot storage, {7 years} in cold storage
...
1. **Attribution**: {Cite this data product in derived works: "Data source:
   {data_product_name} v{version}"}
2. **Usage Constraints**:
...
3. **Data Quality Feedback**: {Report data quality issues via Slack
   #data-support within 24 hours}
...
6. **Rate Limits**: {Stay within allocated rate limits; request increase if
   needed}
7. **Monitoring**: {Monitor your own usage; domain team may throttle excessive
   use}
...
- **Development**: {https://dev-api.example.com} - Schema changes tested here
  first
...
3. {Limitation 3: e.g., "Some legacy records have null values for optional
   fields"}
...
{Full YAML export of this contract in ODCS v3.0.2 format for programmatic
consumption}
...
**Generated by**: ArcKit `/arckit:data-mesh-contract` command **Generated on**:
[DATE] **ArcKit Version**: [VERSION] **Project**: [PROJECT_NAME] (Project
[PROJECT_ID]) **Model**: [AI_MODEL]
